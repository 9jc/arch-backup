...
vpn_disconnect() {
    kitty wg-quick down wg0; myip=$(curl ifconfig.co/) && country=$(curl ifconfig.co/country) && city=$(curl ifconfig.co/city) && notify-send -i ~/.icons/forest-icons/48x48/status/changes-allow.svg "🔥 Wireguard VPN Disconnected" \ "🗲 Current Ip ${myip} \n ✰ ${country} - ${city}"
}
1
