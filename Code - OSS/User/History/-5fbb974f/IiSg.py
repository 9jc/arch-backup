    ...
    if k == 1:
        ...
        for x in range(1, 24):
            try:            
                ...
                # Download Image
                ask_user_to_download = input("Do you want to download the images(d) / quit(q) ?")
                if ask_user_to_download == "no" or "n" or "q":
                    wallpaper_fun(k=2)
                elif ask_user_to_download == "y" or "yes":
                    res = requests.get(image_link_nsfw, stream = True)
#                          
                    if res.status_code == 200:
                        with open(f"{search_query}_name",'wb') as f:
